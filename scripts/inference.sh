...
PARQUET="/data_ali/shunian/data/rl_data/iceberg/test.parquet"
...
MODEL_PATHS=(
    ...
    "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_normal_qwen7b_data_4000_lr1en6_rn4_bs64_4096_1.0/global_step_900/actor/huggingface/"
           )
# 可选：为不同模型指定不同测试集（若设置，则覆盖上面的 MODEL_PATHS/全局 PARQUET）
# 每个元素格式："<模型路径>::<parquet 路径>"；如未提供 parquet 则回退到全局 PARQUET
# 示例：
MODEL_ITEMS=(
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_simple_first7layers_llava_editloss/global_step_300/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_simple.parquet"
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_simple_first7layers_wizard_editloss/global_step_300/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_simple.parquet"
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_extra_simple_Qwen2.5-VL-7B-Instruct_no_kl_bs512_prefil/global_step_300/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_extra_simple.parquet"
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_simple_Qwen2.5-VL-7B-Instruct_no_kl_bs512_prefil/global_step_300/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_simple.parquet"
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_thinking_Qwen2.5-VL-7B-Instruct_no_kl_bs512_prefil/global_step_300/actor/huggingface/::data_ali/shunian/data/rl_data/iceberg/test_thinking.parquet"
  # "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_extra_simple_Qwen2.5-VL-3B-Instruct/global_step_300/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_extra_simple.parquet"
  "/data_ali/shunian/verl/checkpoints/verl_grpo_example_iceberg/qwen2_5_vl_7b_normal_qwen7b_data_4000_lr1en6_rn4_bs64_4096_1.0/global_step_900/actor/huggingface/::/data_ali/shunian/data/rl_data/iceberg/test_normal.parquet"
...
# 收集待处理的 checkpoint 与对应 parquet
CHECKPOINTS=()
PARQUETS=()

# 优先从 MODEL_ITEMS("<model>::<parquet>") 收集；否则回退到 MODEL_PATHS + 全局 PARQUET
if [[ ${#MODEL_ITEMS[@]} -gt 0 ]]; then
...
# 从路径中提取模型名：
# 1) 若包含 global_step_XXX，则取其前一段作为模型名（样例路径适配）
# 2) 回退：若存在携带关键字的段（兼容旧命名：function_rm_iceberg），返回该段
# 3) 再回退：若末段为 huggingface/actor，则尝试取上游目录名
# 4) 最终回退：basename
extract_model_name() {
  local p="$1"
  local IFS='/'
  ...
  # 3) 若以 huggingface/actor 结尾，取上游目录名
  local base
  base="$(basename "${p}")"
  if [[ "${base}" == "huggingface" || "${base}" == "actor" ]]; then
    ...
    # 再往上一层（例如 .../<model>/global_step_xxx/actor/huggingface）
    local grand
...
if [[ ! -f "${INDEX_TSV}" ]]; then
  echo -e "name\tsource_path\tmodel_dir\tparquet" > "${INDEX_TSV}"
fi
...
for idx in "${!CHECKPOINTS[@]}"; do
  ...
  if [[ -z "${parquet}" || ! -e "${parquet}" ]]; then
    echo "[错误] parquet 不存在: ${parquet} (模型: ${ckpt_abs})" >&2
    continue
  fi
  ...
  # 推理并保存日志（带上数据集标识避免同模型不同数据集互相覆盖）
  dtag="$(dataset_tag "${parquet}")"
  ...
  # 维护索引
  if ! grep -Fq "${name_ds}\t${ckpt_abs}\t${model_dir}\t${parquet}" "${INDEX_TSV}"; then
    echo -e "${name_ds}\t${ckpt_abs}\t${model_dir}\t${parquet}" >> "${INDEX_TSV}"
  fi
done
